...
import PredictModal from "../../../lib/components/modals/predict/predict_modal.svelte";
  ...
  async function submitForm(event: SubmitEvent) {
    ...
    if (response.success) {
      body = JSON.parse(response.success);
      body = body.result;
...
<PredictModal bind:values={modal} bind:val={val} bind:data_binded={data}/>
<div class="">
  <div class="flex flex-col">
    <div class="flex flex-row mt-5">
      ...
      <button class="btn" on:click={()=>modal=!modal} >Select Model</button>
    </div>
      ...
      <div class="w-4/12">
        <div class="mockup-code bg-success h-[60vh] text-black">
          ...
          <pre data-prefix="2"><code
            ...
            ></pre>
          <pre data-prefix="3"><code>Suhu Min       : {suhu_min} (Celcius)</code
            ></pre>
          <pre data-prefix="4"><code>Suhu Max       : {suhu_max} (Celcius)</code
            ></pre>
          <pre data-prefix="5"><code>Suhu Avg       : {suhu_avg} (Celcius)</code
            ></pre>
          ...
          <pre data-prefix="9"><code>Penyinaran     : {penyinaran} (%)</code
            ></pre>
          <pre data-prefix="10"><code>Produktivitas  : {produktivitas} (%)</code
            ></pre>
          ...
          <pre data-prefix="14"><code>Model path     : {val.slice(7)}</code></pre>
          <div class="flex flex-col">
            {#if show}
              <div class="flex flex-row">
                <pre data-prefix=">" class="text-warning" />
                <code class="ml-[-20px]"
                  ><p transition:typewriter>Sending Data to server...</p></code
                >
              </div>
            {/if}
            {#if show2}
